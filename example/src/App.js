...
export default class App extends React.Component {
  ...
  setUpCustomMessages = () => {
    ...
    var config = {
      ACCURA_ERROR_CODE_MOTION: this.language == 'en' ? 'Keep Document Steady' : 'حافظ على ثبات المستند',
      ACCURA_ERROR_CODE_DOCUMENT_IN_FRAME: this.language == 'en' ? 'Keep document in frame' : 'احتفظ بالمستند في الإطار',
      ACCURA_ERROR_CODE_BRING_DOCUMENT_IN_FRAME: this.language == 'en' ? 'Bring card near to frame' : 'إحضار البطاقة بالقرب من الإطار',
      ACCURA_ERROR_CODE_PROCESSING: this.language == 'en' ? 'Processing…' : 'يعالج…',
      ACCURA_ERROR_CODE_BLUR_DOCUMENT: this.language == 'en' ? 'Blur detect in document' : 'كشف التمويه في المستند',
      ACCURA_ERROR_CODE_FACE_BLUR: this.language == 'en' ? 'Blur detected over face' : 'تم الكشف عن ضبابية على الوجه',
      ACCURA_ERROR_CODE_GLARE_DOCUMENT: this.language == 'en' ? 'Glare detect in document' : 'كشف الوهج في المستند',
      ACCURA_ERROR_CODE_HOLOGRAM: this.language == 'en' ? 'Hologram Detected' : 'تم الكشف عن صورة ثلاثية الأبعاد',
      ACCURA_ERROR_CODE_DARK_DOCUMENT: this.language == 'en' ? 'Low lighting detected' : 'تم الكشف عن إضاءة منخفضة',
      ACCURA_ERROR_CODE_PHOTO_COPY_DOCUMENT: this.language == 'en' ? 'Can not accept Photo Copy Document' : 'لا يمكن قبول مستند نسخ الصور',
      ACCURA_ERROR_CODE_FACE: this.language == 'en' ? 'Face not detected' : 'لم يتم الكشف عن الوجه',
      ACCURA_ERROR_CODE_MRZ: this.language == 'en' ? 'MRZ not detected' : 'لم يتم الكشف عن MRZ',
      ACCURA_ERROR_CODE_PASSPORT_MRZ: this.language == 'en' ? 'Passport MRZ not detected' : 'لم يتم الكشف عن MRZ جواز سفر',
      ACCURA_ERROR_CODE_ID_MRZ: this.language == 'en' ? 'ID card MRZ not detected' : 'لم يتم الكشف عن بطاقة الهوية MRZ',
      ACCURA_ERROR_CODE_VISA_MRZ: this.language == 'en' ? 'Visa MRZ not detected' : 'لم يتم الكشف عن Visa MRZ',
      ACCURA_ERROR_CODE_WRONG_SIDE: this.language == 'en' ? 'Scanning wrong side of document' : 'مسح الجانب الخطأ من المستند',
      ACCURA_ERROR_CODE_UPSIDE_DOWN_SIDE: this.language == 'en' ? 'Document is upside down. Place it properly' : 'المستند مقلوب. ضعه بشكل صحيح',

      IS_SHOW_LOGO: false,
      SCAN_TITLE_OCR_FRONT: this.language == 'en' ? 'Scan Front Side of' : 'مسح الجانب الأمامي من',
      SCAN_TITLE_OCR_BACK: this.language == 'en' ? 'Scan Back Side of' : 'مسح الجانب الخلفي من',
      SCAN_TITLE_OCR: this.language == 'en' ? 'Scan' : 'مسح',
      SCAN_TITLE_BANKCARD: this.language == 'en' ? 'Scan Bank Card' : 'مسح البطاقة المصرفية',
      SCAN_TITLE_BARCODE: this.language == 'en' ? 'Scan Barcode' : 'مسح الرمز الشريطى',
      SCAN_TITLE_MRZ_PDF417_FRONT: this.language == 'en' ? 'Scan Front Side of Document' : 'مسح الوجه الأمامي للمستند',
