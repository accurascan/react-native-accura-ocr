 ...
 class AccuraScanViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
 ...
 extension AccuraScanViewController: VideoCameraWrapperDelegate {
...
//                }
 ...
 //     }
     ...
     func dlPlateNumber(_ plateNumber: String!, andImageNumberPlate imageNumberPlate: UIImage!) {
         ...
         if let frontUri = AccuraOcr.getImageUri(img: imageNumberPlate, name: nil) {
         ...
         callBack!([NSNull(), AccuraOcr.convertJSONString(results: results)])
         closeMe()
     }
 ...
 //            resultmodel.arrayocrBackSideDataValue = ocrBackModeResult!.arrayocrBackSideDataValue
         ...
         if let faceUri = AccuraOcr.getImageUri(img: resultmodel.faceImage, name: nil) {
             results["face"] = faceUri
         }
         if let frontUri = AccuraOcr.getImageUri(img: resultmodel.frontSideImage, name: nil) {
             results["front_img"] = frontUri
         }
         if let backUri = AccuraOcr.getImageUri(img: resultmodel.backSideImage, name: nil) {
             ...
             if let k = data.key as? String {
                 if k == "Signature" {
                     if let base64img = data.value as? String {
                         ...
                         if let sigUri = AccuraOcr.getImageUri(img: decodedimage, name: nil) {
     ...
     func recognizeSucceedBarcode(_ message: String!, back BackSideImage: UIImage!, frontImage FrontImage: UIImage!, face FaceImage: UIImage!) {
         ...
         if(!isBarcodeEnabled) {
             ...
             if cardType == 1 {
                 if FaceImage != nil{
                     if let frontUri = AccuraOcr.getImageUri(img: FaceImage!, name: nil) {
                         results["face"] = frontUri
                     }
                 }
                 if FrontImage != nil{
                     if let frontUri = AccuraOcr.getImageUri(img: FrontImage!, name: nil) {
                         results["front_img"] = frontUri
                     }
                 }
                 if BackSideImage != nil{
                     if let frontUri = AccuraOcr.getImageUri(img: BackSideImage!, name: nil) {
     ...
     func sendRecognizResults(face: UIImage?, front: UIImage?, back: UIImage?) {
         ...
         if let recogFrontImg = face{
             if let frontUri = AccuraOcr.getImageUri(img: recogFrontImg, name: nil) {
                 results["face"] = frontUri
             }
         }
         ...
         if let recogBackImg = back{
             if let frontUri = AccuraOcr.getImageUri(img: recogBackImg, name: nil) {
                 results["back_img"] = frontUri
             }
         }
         results["front_data"] = frontData
     ...
     func recognizSuccessBankCard(_ cardDetail: NSMutableDictionary!, andBankCardImage bankCardImage: UIImage!) {
         ...
         if let frontUri = AccuraOcr.getImageUri(img: bankCardImage, name: nil) {
             results["front_img"] = frontUri
         }
