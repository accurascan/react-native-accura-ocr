...
class AccuraOcr: NSObject {
    ...
    return ["is_active_accura_ocr": true]
    }
     ...
     func getMetaData(_ callback: @escaping RCTResponseSenderBlock) {
         ...
         DispatchQueue.main.async {
             ...
             self.goNativeCallBack!([NSNull(), AccuraOcr.convertJSONString(results: results)])
         }
     }
     ...
     func startMRZ(_ argsNew: NSArray, callback: @escaping RCTResponseSenderBlock) {
         ...
         gl.type = "mrz"
         DispatchQueue.main.async {
             let viewController = UIStoryboard(name: "MainStoryboard_iPhone", bundle: nil).instantiateViewController(withIdentifier: "AccuraScanViewController") as! AccuraScanViewController
